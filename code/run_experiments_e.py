...
def check_one_schedule(partial_name):
    subprocess.call(['python3', 'main.py', '../test_data/haverfordStudentPrefs.txt', '../test_data/haverfordConstraints_1.txt', '../test_data/haverfordConstraints_2.txt', partial_name+'_extension_studentprefs.txt', '--extension', '-o', partial_name+'_extension_schedule.txt'])
    ...
    # all_students = read_extension_prefs(partial_name + "_extension_studentprefs.txt", all_classes)
    # count_prefs(all_classes, all_students)
    ...
    # print_schedule_extension(schedule, partial_name + "_extension_schedule.txt", all_times)
...
"""
num_tests = 20 #change this number to however many experiments you want to run
ns = 5000
...
for i in range(1, num_tests+1):
    ...
    # make random input
    subprocess.call(["perl", "make_random_input_e_student.pl", str(ns), "../test_data/"+str(i)+"_extension_studentprefs.txt"])
