...
\subsubsection{Extension Implementation}
We add five additional constraints on scheduling:
\begin{enumerate}
\item Assign core classes for each department.
\item Assign popularity to time slots in order to schedule popular class at more popular times.
\item parse out the class times currently used by Haverford College in order to simulate the situation when class times overlap.
\item Teachers have personal time conflicts.
\item A class is possible to have a lab session, which will be assigned in the schedule.
\end{enumerate}
%describe our constraints and explain why they are well-reasoned.
We want to assign core classes because each department, especially those are popular, are highly demanded each semester. By assigning core classes to classes, we think it will be helpful to predict popular classes before the registration and assign big classroom and good time slot for them. Also, since in the real life, most people prefer classes between 10:00 am and 2:00pm, we will set the classes within this time period for larger classroom. Besides that, we want to add on a constraints which contains teachers' personal time conflicts, because some professors are father/mother, so probably can not be able to have class between 8:00 am to 10:00 am and 3:00 pm to 5:00 pm, etc. In order to simulate the schedule in real life. The last constraint add lab session, which is very normal in our college, so we believe add this constraint will make our schedule be closer to real life situation. All the constraints we add are well-reasoned because we know the purpose of this project is to come up implementable recommendations, so we want all the constraints we used should be well-simulated the reality to get optimal result.\\
%talk about how we implement the constraints
We need to collect a lot of more information after we add those constraints in our schedule program. Hence, we add a new parse function in our parser to parse out class times, room name and capacity, class index, class level, class department(with people in that department), has lab session or not, etc, from the HaverfordEnrollment.csv file. Also, since we don't have the information about whether a class is core and when professor will be unavailable, we randomly assign these attributes when we parse. \\
We then modify the attribute of course object to let each course contains the additional information we parse out and store all the courses into a dictionary. Since we don't have student preference information anymore, we have a sort-class function to evaluate the popularity of a class based on its level, its department, whether it is core. We also modify our time table. We sort the time table to store time between 10:00 am to 2:00 pm to the beginning of time table. Besdies, we separate the time table into two dictionaries: lecture-time and lab--time, because normally lab session will take 2.5 to 3 hours in one day, and normal lecture will take 1 - 1.5 hour in two or three days. We don't want them to be messed up with each other. We have a function to test whether a time fit into the current schedule of a room. Finally, the method we schedule the class at the beginning of our sorted class list, to the largest room at most popular time. We still have a queue to store the time or room that is not satisfied for the current class, we will set next class to the time or room in queue if it is available for that class. Since this time we have labs, if a class has lab, we will assign the lab into the schedule right after we schedule this class. There is another function schedule lab, which works similarly as the one
